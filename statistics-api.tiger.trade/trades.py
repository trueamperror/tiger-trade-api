#!/usr/bin/env python3
"""
Tiger Trade API - Trades Module (/trades endpoint)
"""
...
class TradesAPI:
    ...
    def _test_token(self, token: str) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/trades/categories",
                headers=headers,
                timeout=10
            )
    ...
    def get_trades(self, page: int = 1, items_per_page: int = 20, **filters) -> Dict[str, Any]:
        params = {
            "page": page,
            "items_per_page": items_per_page,
            "sort_by": TRADES_PARAMS.get("sort_by", "id"),
            "sort_order": TRADES_PARAMS.get("sort_order", "desc")
        }
    ...
    def get_categories(self) -> Dict[str, Any]:
        return self._make_request("GET", "/trades/categories")
    
    def get_trade_orders(self, trade_id: int) -> Dict[str, Any]:
        return self._make_request("GET", f"/trades/{trade_id}/orders")
    
    def close_trade(self, trade_id: int) -> Dict[str, Any]:
        return self._make_request("POST", f"/trades/{trade_id}/close")


def main():
    print("Tiger Trade Trades - /trades endpoint")
    print("-" * 37)
    
    try:
        api = TradesAPI()
        
        print("Endpoint: /trades")
        print(f"Page: {TRADES_PARAMS['page']}")
        print(f"Items per page: {TRADES_PARAMS['items_per_page']}")
        
        result = api.get_trades(
            page=TRADES_PARAMS['page'],
            items_per_page=TRADES_PARAMS['items_per_page']
        )
        
        if result and isinstance(result, dict):
            if 'data' in result:
                trades = result['data']
                total = result.get('total', len(trades))
                ...
                for trade in trades[:5]:  # Show first 5
                    if isinstance(trade, dict):
                        print(f"ID: {trade.get('id')}, Symbol: {trade.get('symbol')}, "
                              f"Side: {trade.get('side')}, PnL: {trade.get('pnl', 'N/A')}")
            else:
                print("Raw response:")
